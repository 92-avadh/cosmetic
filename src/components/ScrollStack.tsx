...
const ScrollStack: React.FC<ScrollStackProps> = ({
  ...
  const setupLenis = useCallback(() => {
    if (useWindowScroll) {
      // Listen to global window scroll events
  ...
  useLayoutEffect(() => {
    ...
    // In window scroll mode, we look for cards in the document, otherwise in the local container
    const cards = Array.from(
      (useWindowScroll
        ? document.querySelectorAll('.scroll-stack-card')
        : scroller?.querySelectorAll('.scroll-stack-card') || []) as HTMLElement[]
    );
